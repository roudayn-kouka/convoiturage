...
function ReservationsList({ passenger }) {
    ...
    const fetchReservations = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/v1/reserver/consulter",
          {
            headers: {
  ...
  const handleCancelClick = async(index) => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/v1/reserver/annuler/${tripToDelete._id}`, {
          method: 'PATCH',
          headers: {
    ...
    setTrips((prevTrips) => {
      ...
      updatedTrips[index] = { ...updatedTrips[index], status: 'Annulée' };
      return updatedTrips;
    });
              ...
              <div className="card-body">
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div className="d-flex align-items-center">
                    <img
                      src={trip.offer.driver.image}
                      alt={trip.offer.driver.name}
                      className="rounded-circle border border-success me-3"
                      style={{ width: "50px", height: "50px" }}
                    />
                    <div>
                      <h5 className="card-title mb-1">{trip.offer.driver.name}</h5>
