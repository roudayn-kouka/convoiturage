...
const ProfilCond = ({ passenger }) => {
  ...
  useEffect(() => {
    if (passenger) {
      setProfile({
        ...
        password: "",
        confirmPassword: "",
        solde: passenger.solde,
        montantpaye: passenger.montant_payé,
        platformCommission: passenger.commission_plateforme || "45",
      });
    }
  }, [passenger]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put("http://localhost:3000/api/v1/auth_covoitureur/update", profile);
  
      // Check if email or password is changed, then navigate
      if (profile.email !== passenger.email || profile.password) {
        setSuccessMessage("Profil mis à jour avec succès !");
        setEditMode(false);
        navigate("/signincov"); // Redirect to the sign-in page
      } else {
        setSuccessMessage("Profil mis à jour avec succès !");
        setEditMode(false);
      }
